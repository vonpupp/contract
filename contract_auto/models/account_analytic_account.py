...
class AccountAnalyticAccount(models.Model):
    ...
    def _do_auto_pay(self, invoice):
        ...
        self.__send_invoice_message(invoice)
        self.__pay_invoice(invoice)

    @api.multi
    def __pay_invoice(self, invoice):
        ...
        try:
            ...
            if not result or transaction.state not in valid_states:
                _logger.debug(
                    'Payment transaction failed (%s)' % tx.state_message,
                )
            else:
                # Success
                return True
    ...
    @api.multi
    def __get_tx_vals(self, invoice):
        ...
        return {
            ...
            'partner_state': partner.state.name,
            'partner_city': partner.city,
            'partner_street': partner.state,
            'partner_zip': partner.zip,
            'partner_email': partner.email,
        }

    @api.multi
    def __send_invoice_message(self, invoice):
